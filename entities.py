...
class Pod:
    ...
    def resource_status(self):
        ...
        for container in self._containers:
            name = container.name() + container.suffix()
            status = self._client.api.inspect_container(name)
            s['mem'] += self._client.containers.get(status.get('ID', status.get('Id', None))).stats(stream=False)['memory_stats']['usage']
            percpu_usage = self._client.containers.get(status.get('ID', status.get('Id', None))).stats(stream=False)['cpu_stats']['cpu_usage']['percpu_usage']
            for i in range(0,12):
